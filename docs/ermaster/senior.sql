...
D  删除 Delete',
	...
	PRIMARY KEY (guid),
	UNIQUE (guid)
) COMMENT = '代码提交历史 : 记录开发人员在分支上的代码提交历史';
...
ALTER TABLE s_delivery_list
	ADD FOREIGN KEY (guid_program)
	REFERENCES s_program (guid)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;


ALTER TABLE s_program_commit
	ADD FOREIGN KEY (guid_program)
	REFERENCES s_program (guid)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
...
ALTER TABLE s_program
	ADD FOREIGN KEY (guid_project)
	REFERENCES s_project (guid)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
;
